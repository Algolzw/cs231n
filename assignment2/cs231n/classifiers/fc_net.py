...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims) + 1):
            ...
            next_dim = num_classes if i == len(hidden_dims) else hidden_dims[i]
            self.params['W%d' % (i+1)] = np.random.normal(loc=0.0, scale=weight_scale,
                                                     size=(prev_dim, next_dim))
    ...
    def loss(self, X, y=None):
        ...
        cache = None
        caches = []
        # (L-1) of affine-norm-relu-dropout layer
        for i in range(1, self.num_layers):
            out, cache = affine_forward(out,
                self.params['W%i' % i], self.params['b%i' % i])
            caches.append(cache)
            if self.normalization=='batchnorm':
        ...
        dout, grads['W%d' % (i+1)], grads['b%d' % (i+1)] = affine_backward(dscores, cache)
        ...
        while i > 0:
            ...
            if self.normalization=='batchnorm':
                cache = caches.pop()
                dout, grads['gamma%d' % i], grads['beta%d' % i] = batchnorm_backward(dout, cache)
            if self.normalization=='layernorm':
                cache = caches.pop()
                dout, grads['gamma%d' % i], grads['beta%d' % i] = layernorm_backward(dout, cache)
            cache = caches.pop()
            dout, grads['W%d' % i], grads['b%d' % i] = affine_backward(dout, cache)
            i -= 1

        # for L2 reglurization
        for i in range(1, self.num_layers + 1):
            loss += 0.5 * self.reg * np.sum(self.params['W%d' % i]**2)
            grads['W%d' % i] += self.reg * self.params['W%d' % i]
            grads['b%d' % i] += self.reg * self.params['b%d' % i]
